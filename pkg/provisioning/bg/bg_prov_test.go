...
var (
	...
	validBootGuardOptions = &BootGuardOptions{
		KeyManifest:        km,
...
// TestSetKM it is impossible by design of GO to set up an invalid
// BootGuardOption structure to generate errors.
func TestSetKM(t *testing.T) {
	_, err := SetKM(validBootGuardOptions)
	if err != nil {
		t.Fatalf("SetKM failed with: %v", err)
	}
}
...
func TestWriteConfigValid(t *testing.T) {
	...
	if err := WriteConfig(tmpfile, validBootGuardOptions); err != nil {
...
func TestWriteBootGuardStructuresValid(t *testing.T) {
	data, err := ioutil.ReadFile(validBIOSImage)
	if err != nil {
		t.Fatalf("Failed to read validBIOSImage with error: %v", err)
	}
	tmpBPMfile, tmpKMfile, tmpACMfile, tmpdir, err := genTmpBGStrucFiles()
	if err != nil {
	...
	if err = WriteBootGuardStructures(data, tmpBPMfile, tmpKMfile, tmpACMfile); err != nil {
		t.Fatalf("Failed to WriteBootGuardStructures with error: %v", err)
	}
...
func TestWriteBootGuardStructuresInvalidBIOSImage(t *testing.T) {
	...
	if err = WriteBootGuardStructures(data, tmpBPMfile, tmpKMfile, tmpACMfile); err == nil {
		t.Fatalf("WriteBootGuardStructures succeeded but it should not.")
	}
}
...
func TestParseFITEntriesValid(t *testing.T) {
	...
	if bytes.Equal(bpm, validBPM) != true {
		t.Fatalf("BPMs are not equal but they should")
	}
	...
	if bytes.Equal(km, validKM) != true {
		t.Fatalf("KMs are not equla but they should")
	}
	...
	if bytes.Equal(acm, validacm) != true {
		t.Fatalf("ACMs are not the same, but they should.")
	}
}
...
func TestPrecalcPCR0Valid(t *testing.T) {
	t.Skip()
}

func TestPrecalcPCR0InvalidData(t *testing.T) {
	t.Skip()
}

func TestPrecalcPCR0InvalidACMPolicySts(t *testing.T) {
	t.Skip()
}

func TestCalculateNEMSizesValid(t *testing.T) {
	t.Skip()
}

func TestCalculateNEMSizesInvalidBIOSImage(t *testing.T) {
	t.Skip()
}

func TestCalculateNEMSizesInvalidBPM(t *testing.T) {
	t.Skip()
}

func TestCalculateNEMSizesInvalidKM(t *testing.T) {
	t.Skip()
}

func TestCalculateNEMSizesInvalidACM(t *testing.T) {
	t.Skip()
}
...
func TestStitchFITEntriesInvalidBIOSImage(t *testing.T) {
	// This test needs a coreboot image especially manipulated to get the job done.
	t.Skip()
	acm, err := ioutil.ReadFile(validACM)
	if err != nil {
		t.Errorf("Read acm failed: %v", err)
	...
	if err := StitchFITEntries(invalidBIOSImage, acm, bpm, km); err == nil {
		t.Errorf("Stitching succeeded, but it shouldn't")
	}
}

func TestStitchFITEntriesInvalidKmBinary(t *testing.T) {
	t.Skip()
	acm, err := ioutil.ReadFile(validACM)
	if err != nil {
		t.Errorf("Read acm failed: %v", err)
	...
	km, err := ioutil.ReadFile(invalidKM)
	if err != nil {
		t.Errorf("Read KM failed: %v", err)
	}
	if err := StitchFITEntries(validBIOSImage, acm, bpm, km); err == nil {
...
func TestStitchFITEntriesInvalidBpmBinary(t *testing.T) {
	t.Skip()
	acm, err := ioutil.ReadFile(validACM)
	if err != nil {
		t.Errorf("Read acm failed: %v", err)
	}
	bpm, err := ioutil.ReadFile(invalidBPM)
	if err != nil {
		t.Errorf("Read BPM failed: %v", err)
	}
	km, err := ioutil.ReadFile(validKM)
...
func TestStitchFITEntriesInvalidAcmBinary(t *testing.T) {
	// Only corrupted ACMs should throw errors
	t.Skip()
	acm, err := ioutil.ReadFile(invalidACM)
	if err != nil {
		t.Errorf("Read acm failed: %v", err)
	}
	bpm, err := ioutil.ReadFile(validBPM)
...
func TestParseKMInvalid(t *testing.T) {
	t.Skip()
	kmbin, err := ioutil.ReadFile(invalidKM)
	if err != nil {
		t.Errorf("Error reading KM: %v", err)
	}
	r := bytes.NewReader(kmbin)
	_, err = ParseKM(r)
	if err == nil {
		t.Errorf("Parsing KM succeeded, but it shouldn't")
	}
}
...
func TestParseBPMInvalid(t *testing.T) {
	t.Skip()
	bpmbin, err := ioutil.ReadFile(invalidBPM)
	if err != nil {
		t.Errorf("Error reading KM: %v", err)
	}
	r := bytes.NewReader(bpmbin)
...
func genTmpKeyFiles() (*os.File, *os.File, *os.File, *os.File, string, error) {
	tmpdir, err := ioutil.TempDir("", "bgTests")
	if err != nil {
...
func genTmpBGStrucFiles() (*os.File, *os.File, *os.File, string, error) {
	tmpdir, err := ioutil.TempDir("", "bgTests")
	if err != nil {
